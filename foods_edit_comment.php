...
if (isset($_POST['edit_comment'])) {
    ...
    // Check if the foods trying to edit the comment is the owner
    $stmt = $conn->prepare("SELECT foods_id FROM laf_comment WHERE laf_comment_id = ?");
    $stmt->bind_param("i", $comment_id);
    $stmt->execute();
    $stmt->bind_result($comment_foods_id);
    $stmt->fetch();
    $stmt->close();

    if ($comment_foods_id != $_SESSION['id']) {
        echo "You do not have permission to edit this comment.";
        exit;
    }

    // Use prepared statement to update the comment
    $stmt = $conn->prepare("UPDATE laf_comment SET content = ? WHERE laf_comment_id = ?");
    ...
    $sql = "SELECT content FROM laf_comment WHERE laf_comment_id = $comment_id";
