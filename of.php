...
$foundersPerPage = 20; // Number of founders to display per page
...
try {
    ...
    $offset = ($page - 1) * $foundersPerPage;

    // Query to get academic founders with pagination
    $queryGetFoundersWithPagination = "SELECT fi.Id_Number, fi.name, fi.department, fi.year, fi.mobile, fi.found_date, fi.found_item
    FROM `founder_information` fi
    WHERE (fi.name LIKE '%$searchQuery%' OR fi.department LIKE '%$searchQuery%' OR fi.year LIKE '%$searchQuery%')
    AND fi.approval_status = 'Accepted'  -- Filter by 'Accepted' status
    ORDER BY fi.found_item DESC LIMIT $offset, $foundersPerPage";

    $result = mysqli_query($conn, $queryGetFoundersWithPagination);
    ...
    if ($result) {
        ...
        // Query to count total number of Founders
        $queryCountFounders = "SELECT COUNT(*) as total FROM `founder_information`
            WHERE name LIKE '%$searchQuery%' OR department LIKE '%$searchQuery%' OR year LIKE '%$searchQuery%'";
        $countResult = mysqli_query($conn, $queryCountFounders);
        ...
        $totalPages = ceil($rowCount / $foundersPerPage);
        ...
        $founders = []; // Initialize an empty array to avoid errors if the query fails
    }
} catch (Exception $e) {
        ...
        <table>
            ...
            <tbody>
                <?php foreach ($founders as $founder): ?>
                    <tr>
                        <td><?php echo $founder['Id_Number']; ?></td>
                        <td><?php echo $founder['name']; ?></td>
                        <td><?php echo $founder['department']; ?></td>
                        <td><?php echo $founder['year']; ?></td>
                        <td><?php echo $founder['mobile']; ?></td>
                        <td><?php echo $founder['found_date']; ?></td>
                        <td><?php echo $founder['found_item']; ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
