...
if (isset($_GET['post_id'])) {
    ...
    $check_author_sql = "SELECT user_id FROM foods_post WHERE foods_post_id = $post_id";
    ...
    if ($check_author_result->num_rows > 0) {
        ...
        if ($post['user_id'] == $_SESSION['admin_id']) {
            ...
            $delete_reaction_logs_sql = "DELETE FROM foods_reaction_logs WHERE foods_post_id = $post_id";
            if ($conn->query($delete_reaction_logs_sql)) {
                // Now that related rows are deleted, delete the post
                $delete_post_sql = "DELETE FROM foods_post WHERE foods_post_id = $post_id";
                if ($conn->query($delete_post_sql)) {
                    // Redirect back to the announcement page or any desired location
                    header('location: admin_foods_announcement.php');
                    exit;
                } else {
