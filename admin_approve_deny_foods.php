...
$query = "SELECT * FROM `foods_post` WHERE `approved` = 0 LIMIT $offset, $recordsPerPage";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['approve'])) {
        ...
        $updateQuery = "UPDATE `foods_post` SET `approved` = 1 WHERE `foods_post_id` = $postId";
        ...
        $deleteQuery = "DELETE FROM `foods_post` WHERE `foods_post_id` = $postId";
...
<div class="container">
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            echo '<input type="hidden" name="post_id" value="' . $row['foods_post_id'] . '">';
            ...
            echo '</form>';
            echo '<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#imageModal' . $row['foods_post_id'] . '">View Image</button>';
            echo '</div>';
            echo '</td>';
            echo '</tr>';

            // Modal for each post
            echo '<div class="modal fade" id="imageModal' . $row['foods_post_id'] . '" tabindex="-1" role="dialog" aria-labelledby="imageModalLabel" aria-hidden="true">';
        ...
        // Pagination links
        $totalRecordsQuery = "SELECT COUNT(*) as total FROM `foods_post` WHERE `approved` = 0";
